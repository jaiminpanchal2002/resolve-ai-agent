...
async def test_rrf_scoring_and_ranking():
    ...
    service.retrieve_semantic = mock.AsyncMock(
        return_value=[(chunk_a, policy_a), (chunk_b, policy_b)]
    )
    # Lexical: A is 2nd (rank 2), B is 1st (rank 1)
    service.retrieve_lexical = mock.AsyncMock(
        return_value=[(chunk_b, policy_b), (chunk_a, policy_a)]
    )
    ...
    # Overlap case: both retrievers return A (rank 1) then B (rank 2)
    service.retrieve_semantic = mock.AsyncMock(
        return_value=[(chunk_a, policy_a), (chunk_b, policy_b)]
    )
    service.retrieve_lexical = mock.AsyncMock(
