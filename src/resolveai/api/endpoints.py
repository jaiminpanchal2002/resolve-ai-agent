...
async def create_ticket(
    ...
    async_run: bool = Query(
        default=False, description="Run agent asynchronously via Celery worker"
    ),
    db: AsyncSession = Depends(get_async_db),
):
...
async def get_ticket_details(
    ...
    stmt_runs = (
        select(AgentRun).where(AgentRun.ticket_id == ticket_id).order_by(AgentRun.started_at.desc())
    )
    ...
    return {
        ...
        "human_review": {
            ...
            "reviewed_at": ticket.human_reviewed_at.isoformat()
            if ticket.human_reviewed_at
            else None,
        }
        if ticket.human_review_status
        else None,
...
async def submit_human_review(
    ...
    ticket.human_reviewed_at = datetime.datetime.now(datetime.UTC)
    ...
    if review_in.status == "APPROVED":
        ...
        ticket.status = "OPEN"
    elif review_in.status == "EDITED" and review_in.edited_resolution:
...
async def get_dashboard_metrics(db: AsyncSession = Depends(get_async_db)):
    ...
    stmt_res = select(func.count(AgentDecision.id)).where(AgentDecision.resolution == "RESOLVED")
    resolved_count = await db.scalar(stmt_res) or 0

    stmt_esc = select(func.count(AgentDecision.id)).where(AgentDecision.resolution == "ESCALATE")
    ...
    try:
        stmt_p95 = select(func.percentile_cont(0.95).within_group(AgentRun.latency_ms.asc()))
