...
def run_evaluation_task(evaluation_run_id: int) -> dict[str, Any]:
    ...
    async def _run():
        async with async_session_factory() as db:
            ...
            stmt_cases = select(EvaluationCase).where(EvaluationCase.dataset_id == eval_run.dataset_id)
            ...
            eval_run.completed_at = datetime.datetime.utcnow()
...
def evaluate_case_performance(expected: dict[str, Any], actual: dict[str, Any]) -> dict[str, float]:
    ...
    actual_intent = actual.get("classification", {}).get("intent") if actual.get("classification") else None
    ...
    actual_tools = set(t["tool"] for t in actual.get("tool_outputs", []) if t.get("status") == "SUCCESS")
    ...
    actual_policy_ids = set(p for p in actual_policies if "POL-" in str(p))
