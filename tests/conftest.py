...
from resolveai.models.models import Base, Customer, Order, Payment, Policy, PolicyChunk, Shipment, Ticket, TicketMessage
...
def test_sync_engine(postgres_container):
    ...
    with engine.begin() as conn:
        conn.execute(Base.metadata.schema.select()) # dummy check or raw sql
...
async def db_session(test_async_engine):
    ...
    async with session_factory() as session:
        # Run inside a savepoint transaction to rollback after test completes
        async with session.begin():
            yield session
            # Transaction auto-rolls back when exiting context block
...
async def seeded_db(db_session):
    ...
    chunk = PolicyChunk(
        ...
        content="Orders above ₹50,000 with missing proof of delivery must be escalated to the logistics investigation team.",
        embedding=[0.1] * 1536,
        chunk_index=0
    )
