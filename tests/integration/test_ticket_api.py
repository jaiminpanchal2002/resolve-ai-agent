...
from resolveai.main import app
from resolveai.models.models import Customer, User
...
async def api_client(db_session: AsyncSession):
    ...
    if not res.scalar_one_or_none():
        user = User(
            email="admin@resolveai.com", hashed_password=get_password_hash("admin123"), role="admin"
        )
        db_session.add(user)
...
async def test_auth_and_ticket_flow(api_client: AsyncClient):
    ...
    login_data = {"username": "admin@resolveai.com", "password": "admin123"}
    ...
    ticket_payload = {
        ...
        "messages": [
            {
                "sender": "customer",
                "body": "My package ORD-123 shows delivered but I never received it.",
            }
        ],
    }
    ...
    create_resp = await api_client.post(
        "/api/tickets?async_run=false", json=ticket_payload, headers=headers
    )
    ...
    review_resp = await api_client.post(
        f"/api/tickets/{ticket_id}/review", json=review_payload, headers=headers
    )
